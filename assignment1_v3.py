...
class MergeSort:
    def __init__(self, arr, ax):
        ...
        self.ax = gridspec.GridSpecFromSubplotSpec(2, 1, subplot_spec=ax, height_ratios=[1, 4], hspace=0.05)
        self.ax_inset = plt.subplot(self.ax[0])
        self.ax_main = plt.subplot(self.ax[1])
        self.tmp = []
        self.tmp_record = []
    ...
    def draw(self,frame=None):
        if len(self.tmp_record) == 0:
            self.ax_main.clear()
            self.ax_main.bar(range(1, len(self.arr) + 1), self.arr, color="skyblue")
            self.ax_inset.clear()
        ...
        if frame is None:
            self.ax_inset.set_title("Merge Sort")
        else:
            self.ax_inset.set_title(f"Merge Sort Frame: {frame}")
